...
class TinkarServiceImplTest {
    ...
    // ── publicIdToSearchResult: calc non-null branch ─────────────────────────

    @Test
    void getEntity_calcInitSucceeds_descLookupThrows_fallsBackToDirectScan() {
        // calc != null (Calculators.View.Default() succeeds), but languageCalculator() throws
        // → descLookupSucceeded stays false → fallback description scan runs
        try (MockedStatic<Calculators.View> calcMock = Mockito.mockStatic(Calculators.View.class);
             MockedStatic<EntityService> entityMock = Mockito.mockStatic(EntityService.class)) {

            ViewCalculatorWithCache mockCalc = Mockito.mock(ViewCalculatorWithCache.class);
            calcMock.when(() -> Calculators.View.Default()).thenReturn(mockCalc);
            when(mockCalc.languageCalculator()).thenThrow(new RuntimeException("language init failed"));

            EntityService mockEs = Mockito.mock(EntityService.class);
            entityMock.when(EntityService::get).thenReturn(mockEs);
            when(mockEs.nidForPublicId(any())).thenReturn(123);
            when(mockEs.semanticNidsForComponent(anyInt())).thenReturn(new int[]{});
            when(mockEs.getEntityFast(anyInt())).thenReturn(null);

            when(primitive.getPublicId(any())).thenReturn(publicId("aaaaaaaa-0000-0000-0000-000000000001"));

            TinkarSearchQueryResponse response = service.getEntity("aaaaaaaa-0000-0000-0000-000000000001");
    ...
    @Test
    void getChildConcepts_calcInitSucceeds_descLookupThrows_stillReturnsResults() {
        // Confirms that when ViewCalculator init succeeds but description lookup throws,
        // the result list still contains the children with empty descriptions.
        try (MockedStatic<Calculators.View> calcMock = Mockito.mockStatic(Calculators.View.class);
             MockedStatic<EntityService> entityMock = Mockito.mockStatic(EntityService.class)) {
            ...
            EntityService mockEs = Mockito.mock(EntityService.class);
            entityMock.when(EntityService::get).thenReturn(mockEs);
            when(mockEs.nidForPublicId(any())).thenReturn(456);
            when(mockEs.semanticNidsForComponent(anyInt())).thenReturn(new int[]{});
            when(mockEs.getEntityFast(anyInt())).thenReturn(null);
    ...
    @Test
    void getDescendantConcepts_calcInitSucceeds_descLookupThrows_stillReturnsResults() {
        try (MockedStatic<Calculators.View> calcMock = Mockito.mockStatic(Calculators.View.class);
             MockedStatic<EntityService> entityMock = Mockito.mockStatic(EntityService.class)) {
            ...
            EntityService mockEs = Mockito.mock(EntityService.class);
            entityMock.when(EntityService::get).thenReturn(mockEs);
            when(mockEs.nidForPublicId(any())).thenReturn(789);
            when(mockEs.semanticNidsForComponent(anyInt())).thenReturn(new int[]{});
            when(mockEs.getEntityFast(anyInt())).thenReturn(null);

            PublicId descId = publicId("dddddddd-0000-0000-0000-000000000001");
            when(primitive.getPublicId(any())).thenReturn(publicId("bbbbbbbb-0000-0000-0000-000000000001"));
            when(primitive.descendantsOf(any())).thenReturn(List.of(descId));

            TinkarSearchQueryResponse response = service.getDescendantConcepts("parent-id");
    ...
    // ── buildStampVersion / setPublicIdIfPresent ──────────────────────────────

    @Test
    @SuppressWarnings("unchecked")
    void getEntity_entityHasVersionsAndStamp_populatesStampTime() {
        try (MockedStatic<Calculators.View> calcMock = Mockito.mockStatic(Calculators.View.class);
             MockedStatic<EntityService> entityMock = Mockito.mockStatic(EntityService.class)) {

            // Calculator init throws the "empty DB" pattern so description falls back gracefully
            calcMock.when(() -> Calculators.View.Default())
                    .thenThrow(new IllegalStateException("No entity key found for UUIDs [test]"));
            ...
            // nidForPublicId used by publicIdToSearchResult
            when(mockEs.nidForPublicId(any())).thenReturn(100);
            // semanticNidsForComponent used by getFallbackDescriptionText
            when(mockEs.semanticNidsForComponent(100)).thenReturn(new int[]{});

            // Build mock stamp entity
            StampEntity<?> mockStamp = Mockito.mock(StampEntity.class);
            when(mockStamp.stateNid()).thenReturn(10);
            when(mockStamp.authorNid()).thenReturn(11);
            when(mockStamp.moduleNid()).thenReturn(12);
            when(mockStamp.pathNid()).thenReturn(13);
            when(mockStamp.time()).thenReturn(12345L);
            when(mockEs.getStampFast(200)).thenReturn((StampEntity) mockStamp);

            // Build mock entity version
            dev.ikm.tinkar.entity.EntityVersion mockVersion = Mockito.mock(dev.ikm.tinkar.entity.EntityVersion.class);
            when(mockVersion.stampNid()).thenReturn(200);

            // Build mock entity with one version
            Entity<?> mockEntity = Mockito.mock(Entity.class);
            Mockito.doReturn(Lists.immutable.of(mockVersion)).when(mockEntity).versions();

            // getEntityFast: nid 100 -> mockEntity; nids 10,11,12,13 -> null (no publicId)
            when(mockEs.getEntityFast(anyInt())).thenAnswer(invocation -> {
                int nid = invocation.getArgument(0);
                return switch (nid) {
                    case 100 -> mockEntity;
                    default -> null;
                };
            ...
            assertThat(response.getSuccess()).isTrue();
            assertThat(response.getResults(0).getStamp().getTime()).isEqualTo(12345L);
        }
    }

    @Test
    @SuppressWarnings("unchecked")
    void getEntity_stampComponentsHavePublicIds_setsAllStampFields() {
        try (MockedStatic<Calculators.View> calcMock = Mockito.mockStatic(Calculators.View.class);
             MockedStatic<EntityService> entityMock = Mockito.mockStatic(EntityService.class)) {
            ...
            when(mockStamp.moduleNid()).thenReturn(12);
            when(mockStamp.pathNid()).thenReturn(13);
            when(mockStamp.time()).thenReturn(99999L);
            when(mockEs.getStampFast(200)).thenReturn((StampEntity) mockStamp);
            ...
            // Stamp-component entities (status=10, author=11, module=12, path=13)
            Entity<?> statusEntity  = Mockito.mock(Entity.class);
            Entity<?> authorEntity  = Mockito.mock(Entity.class);
            Entity<?> moduleEntity  = Mockito.mock(Entity.class);
            Entity<?> pathEntity    = Mockito.mock(Entity.class);

            when(statusEntity.publicId()).thenReturn(publicId("11111111-0000-0000-0000-000000000001"));
            when(authorEntity.publicId()).thenReturn(publicId("22222222-0000-0000-0000-000000000002"));
            when(moduleEntity.publicId()).thenReturn(publicId("33333333-0000-0000-0000-000000000003"));
            when(pathEntity.publicId()).thenReturn(publicId("44444444-0000-0000-0000-000000000004"));

            when(mockEs.getEntityFast(anyInt())).thenAnswer(invocation -> {
                int nid = invocation.getArgument(0);
                return switch (nid) {
                    case 100 -> mockEntity;
                    case 10  -> statusEntity;
                    case 11  -> authorEntity;
                    case 12  -> moduleEntity;
                    case 13  -> pathEntity;
                    default  -> null;
                };
            });
            ...
            assertThat(response.getSuccess()).isTrue();
            assertThat(response.getResults(0).getStamp().getStatusPublicId().getUuids(0))
                    .isEqualTo("11111111-0000-0000-0000-000000000001");
            assertThat(response.getResults(0).getStamp().getAuthorPublicId().getUuids(0))
                    .isEqualTo("22222222-0000-0000-0000-000000000002");
        }
    }
