...
const savedCoordinateGroup: TestGroupDefinition = {
  ...
  tests: [
    ...
    {
      id: 'lc-save',
      name: 'Save language coordinate (GB_ENGLISH_PREFERRED_NAME) — returns content-derived ID',
      run: async (ctx) => {
        const data = await saveLanguageCoordinate({ languagePreset: 'GB_ENGLISH_PREFERRED_NAME' });
        if (!data.id) {
          return { status: 'fail', detail: 'No id in response', responseData: data };
        }
        ctx.set('saved_lang_id', data.id);
        return { status: 'pass', detail: `Saved language coordinate with id: ${data.id}`, responseData: data };
      },
    },
    {
      id: 'lc-save-idempotent',
      name: 'Save same language coordinate again — returns same ID (idempotent)',
      run: async (ctx) => {
        const firstId = ctx.get('saved_lang_id');
        if (!firstId) return { status: 'skip', detail: 'No saved language coordinate ID from previous test' };
        const data = await saveLanguageCoordinate({ languagePreset: 'GB_ENGLISH_PREFERRED_NAME' });
        if (data.id === firstId) {
          return { status: 'pass', detail: `Same ID returned: ${data.id}`, responseData: data };
        }
    ...
    {
      id: 'lc-list',
      name: 'List language coordinates — saved coordinate appears in list',
      run: async (ctx) => {
        const savedId = ctx.get('saved_lang_id');
        const data = await listLanguageCoordinates();
        if (data.length === 0) {
          return { status: 'fail', detail: '0 language coordinates returned', responseData: data };
        }
        if (savedId && !data.some((c) => c.id === savedId)) {
          return {
            status: 'fail',
            detail: `Saved language coordinate (${savedId}) not found in list of ${data.length}`,
            responseData: data,
          };
        }
        return {
          status: 'pass',
          detail: `${data.length} language coordinate(s) found, saved ID present`,
          responseData: data,
        };
      },
    },
